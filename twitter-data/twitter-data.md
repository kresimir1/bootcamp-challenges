...
We've obtained some data from the Twitter API to learn more about some Launchers! Now we want to use the data to answer some questions.
...
1. Each username followed by its description. If there is no description (i.e., `nil` then print out "NA" instead):
  ...
  ```no-highlight
  LaunchAcademy: A 10 week, intensive bootcamp teaching you how to code with a focus on Ruby on Rails
...
4. Each username followed by the total character count used in that user's last twenty tweets.
  ...
  ```no-highlight
  LaunchAcademy used 2431 characters in their last twenty tweets.
...
5. Write Ruby code to answer each of the following questions about our Twitter data.
...
How would I find the description of `LaunchAcademy` in `hash`? First, I want to isolate `LaunchAcademy`:
