...
#YOUR CODE GOES HERE
# 1. Output each username followed by its description.
# If there is no description (i.e., nil then print out "NA" instead):
# 2. Each username followed by total number of followers.
output_categories = ["description", "number of followers"]
output_categories.each do |output_category|
  puts "A list of users and #{output_category}:"
  twitter_data.each do |userdata|
    userdata.each do |username, categories|
      if categories[output_category].nil?
        categories[output_category] = "NA"
      end
      puts " #{username} : #{categories[output_category]}"
    end
  end
  puts
end
...
twitter_data.each do |userdata|
  userdata.each do |username, categories|
    last_tweet_count = categories["last twenty tweets"][0].size
  puts "#{username} had #{last_tweet_count} characters used in their last tweet."
  end
end
puts
#4. Each username followed by the total character count used in that user's
# last twenty tweets.
twitter_data.each do |userdata|
  userdata.each do |username, categories|
    character_count = 0
    categories["last twenty tweets"].each do |tweets|
      character_count += tweets.size
    end
  puts "#{username} had #{character_count} characters used in their last twenty tweets."
  end
end
puts

#5. Which user has the most followers?
#6. Which user has the most friends?
#7. Which user has the greatest number of tweets?
questions_category = ["number of followers", "number of friends", "number of tweets"]
questions_category.each do |question|
  most_number = 0
  most_username = ""
  twitter_data.each do |userdata|
    userdata.each do |username, categories|
      if categories[question].to_i > most_number
        most_number = categories[question].to_i
        most_username = username
      end
    end
  end
  puts "#{most_username} has the largest #{question}: #{most_number}"
end
puts
#8. Which users have a description listed in twitter_data?
#9. Which users have a location listed in twitter_data?
categories_present = ["description", "location"]
categories_present.each do |category_present|
  puts "Users that have #{category_present} listed: "
  twitter_data.each do |userdata|
    userdata.each do |username, categories|
      if !categories[category_present].nil?
        puts username
      end
    end
  end
